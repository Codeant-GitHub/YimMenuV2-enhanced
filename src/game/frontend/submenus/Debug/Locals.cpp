...
namespace YimMenu::Submenus
{
	...
	std::shared_ptr<Category> BuildLocalsMenu()
	{
		...
		script->AddItem(std::make_unique<ImGuiItem>([] {
			...
			if (ImGui::BeginCombo("Thread", cur_thread ? cur_script_name : "(Select)"))
			{
				for (auto script : *Pointers.ScriptThreads)
				{
					if (script)
					{
						if (script->m_Context.m_State == rage::scrThread::State::KILLED || script->m_Context.m_StackSize == 0)
							continue;

						ImGui::PushID(script->m_Context.m_ThreadId);

						auto script_name = script->m_ScriptName;
			...
			if (cur_thread && (cur_thread->m_Context.m_ScriptHash != cur_script_hash || cur_thread->m_Context.m_StackSize == 0 || cur_thread->m_Context.m_State == rage::scrThread::State::KILLED))
				cur_thread = nullptr;
		...
		saved->AddItem(std::make_unique<ImGuiItem>([] {
			...
			auto script_idf = cur_thread->m_ScriptHash;
