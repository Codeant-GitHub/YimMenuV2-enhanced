...
namespace YimMenu::Features
{
	class DLCVehicles : public BoolCommand
	{
		...
		virtual void OnEnable() override
		{
			...
			{
				m_VehiclePOSIXPatch = ScriptPatches::AddPatch("appinternet"_J, "59 ? ? 72 2E 02 01", 0, {0x2B, 0x00, 0x00});
			}
			m_VehiclePOSIXPatch->Enable();

			if (!m_GetVehiclePricePatch)
			{
				m_GetVehiclePricePatch = ScriptPatches::AddPatch("appinternet"_J, "56 ? ? 70 2E 04 01 38 01", 0, {0x55});
			}
			m_GetVehiclePricePatch->Enable();

			if (!m_BuyVehiclePatch)
			{
				m_BuyVehiclePatch = ScriptPatches::AddPatch("appinternet"_J, "5D ? ? ? 06 56 ? ? 38 00 25 ? 50", 5, {0x55});
			}
